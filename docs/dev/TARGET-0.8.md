...
A human assignment is exact: player identity -> logical slot. The user chooses exactly which bot intent a human replaces. That logical player slot is suppression/composition intent, not a claim about the human's physical Blizzard row: in a party every player sees themselves as party member 1, and raid human placement is not reliable enough to treat as editor-controlled. Bot order is different: SCB's summoner can control bot order within a group, so bot logical ordering remains meaningful and should stay deterministic.
...
Post-consolidation extension approved 2026-09-18: expose the same logical-slot editor at every preset size. A preset is N underlying bot/composition intents. Dragging a player onto a slot chooses exactly which bot intent that player suppresses, without pretending to choose that player's Blizzard row. Bot intents should also be draggable between preset slots so the user can swap/reorganise composition and therefore the controlled bot summon/order. Party versus raid changes size and topology, not the editor concept. Human placement remains observational; bot order remains intentionally controlled by the summoner. Implement this only after structural consolidation is complete, then runtime-test it before the final 0.8 regression/main promotion.

### Player logical slot is not Blizzard row
Blizzard roster/order remains the source of truth for live human placement. SCB must never assume a player's logical preset slot equals that player's displayed Blizzard row. In party UI each player sees themselves as member 1; in raids human subgroup/row placement is not reliably controllable.

That limitation is specific to players. Bot order within a group is controllable through SCB's summoner and remains part of the logical composition model. Bot identity/order is resolved while ignoring human row placement: the explicit burst plan and reliable bot-join messages establish bot name -> logical assignment/order, and Blizzard roster verifies the resulting live names/classes/layout. Human insertion must not redefine which logical bot intent a player suppresses.
