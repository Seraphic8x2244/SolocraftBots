...
A human assignment is exact: player identity -> logical slot. The UI may visually group slots in blocks of five for larger presets, but that grouping is composition intent only. The user chooses exactly which bot intent a human replaces; SCB must not imply that it can enforce the human's or bot's physical party/raid position.
...
Post-consolidation extension approved 2026-09-18: expose the same exact logical-slot model at every preset size. A preset is simply N underlying bot/composition intents; dragging a player onto a slot chooses exactly which bot intent that player suppresses. Bot intents should also be draggable between preset slots so the user can swap/reorganise the logical composition. Party versus raid changes the number of logical slots, not the editor model. SCB must not present a logical slot or five-slot visual block as control over Blizzard's physical party order or raid subgroup placement. Implement this only after structural consolidation is complete, then runtime-test it before the final 0.8 regression/main promotion.
