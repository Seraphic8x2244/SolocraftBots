...
Baseline: 0.7.14
Current live build: 0.8.20-dev. The 0.8.14 rebuild handoff and 0.8.15 removal-settle placement remain runtime-proven. 0.8.18 introduced one top-level bot operation identity and passed normal 5-man, 5 -> 5, 5 -> 10 survivor/conversion and in-flight 10-man replacement tests. 0.8.19 moved pending-add recovery, rebuild teardown/settle and next-frame handoff state into `botOperation.rebuild`; repeated deliberately unreasonable Ctrl-click interruptions of 10-bot dungeon operations still resolved to the latest requested preset without reported errors or stuck state. 0.8.20 now moves survivor/bootstrap handoff state into `botOperation.safety` behind a one-gate compatibility hydration bridge; runtime verification of that state migration is pending. The final Spawn target remains one authoritative bot-lifecycle operation coordinator used by preset summon/rebuild and maintenance replacement execution.
...
0.8.11-dev absorbed the former standalone `PresetRebuild.lua` transition barrier into `Spawn.lua`, then 0.8.12-dev rolled that specific move back after a runtime client hang. The separate handoff was re-proven in 0.8.14-dev and the next-add settle placement passed in 0.8.15-dev. 0.8.18 introduced transitional `SpawnOperation.lua` around the proven runtime. 0.8.19 moved rebuild/pending-add/settle ownership into `botOperation.rebuild` while preserving the 0.8.14 next-frame handoff, and aggressive Ctrl-switch testing passed. 0.8.20 moves survivor/bootstrap handoff state into `botOperation.safety`; the old field names are hydrated only during a scheduler frame for compatibility. After that gate passes, the coordinator implementation should be absorbed into `Spawn.lua` and those compatibility globals removed rather than retained as a parallel state model.
