...
- Branch: `dev`
- Version: `0.8.63-dev`
- Current runtime commit: `541821f7e2cf6e27f7f51ce275f3b5befe038aad`
- Latest status commit before this update: `676d20680b4820e1b9fe23eb3fda76199b95cad6`
...
## Recent Commits
- `541821f7e2cf6e27f7f51ce275f3b5befe038aad` — 0.8.63-dev: Group command startup now refreshes the live roster once before resolving target subgroup and recipient names.
- `676d20680b4820e1b9fe23eb3fda76199b95cad6` — recorded the decision to progress past a dedicated 0.8.62-only timing gate.
...
- Existing raid preset logic records exact logical human slot occupancy independently of Blizzard's physical row/order.
...
- 0.8.63-dev keeps Group-row UI refresh event-driven/cached, but an actual Group command calls `SCB_GetTargetLiveGroup(true)`, forcing one fresh `SCB_GetLiveRoster(true)` snapshot before subgroup/recipient resolution.
- Queue timing, rolling 24 commands/second accounting, Group critical section, subgroup filtering, GUILD transport and original-target restoration are unchanged.
- Ctrl-click Group Come still sends Move + Come back-to-back in the same recipient send phase.
- 0.8.61-0.8.63 Group changes remain implemented but not yet user-verified; the user accepted combining their validation into the next natural Group test.
...
- Not tested: 0.8.61-dev through 0.8.63-dev Group refinements.

### Next Test
- Exercise normal Group Come in both 4-bot and 5-bot subgroups on 0.8.63-dev.
- Re-test immediately after a roster change so the fresh snapshot path is exercised.
...
- Reuse existing tracker/preset slot data and proven refill semantics where possible; avoid duplicating raid logic in a party-only path.
...
- Dedicated 0.8.62-only timing validation is deferred; its behaviour will be covered with 0.8.63.

## Exact Next Step
Audit the current 5-player preset slot representation, raid human-slot representation, preset-to-Active-Roster handoff and Replace Missing record generation. Identify the smallest reuse path that preserves an underlying bot intent when a human occupies that exact logical slot, then implement that isolated data-model step before changing editor UI or maintenance spawning behaviour.
