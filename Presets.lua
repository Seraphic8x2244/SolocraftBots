-- SoloCraft Bots - Presets
-- Preset data, editor, summoning, tracking/refill, tutorial, and panel UI.
                ...
                if slot then
                    ...
                    if row.blessingButton then
                        ...
                        elseif slot.class == "shaman" then
                            ...
                            if row.blessingButton.totemIcons then
                                for totemKey = 1, 4 do
                                    local groupKey = SCB.SHAMAN_TOTEM_ORDER[totemKey]
...
function SCB_PresetClassOnClick()
    ...
    if IsShiftKeyDown and IsShiftKeyDown() then
        ...
        for j = groupStart, groupEnd do
            ...
            if groupSlot then
                ...
                elseif classInfo.key == "shaman" then
                    groupSlot.extra = SCB.DEFAULT_SHAMAN_TOTEMS
                else
                    groupSlot.extra = roleInfo.extra
                end
            end
        ...
        elseif classInfo.key == "shaman" then
            slot.extra = SCB.DEFAULT_SHAMAN_TOTEMS
        else
...
function SCB_PresetRoleOnClick()
    ...
    if IsShiftKeyDown and IsShiftKeyDown() then
        ...
        for j = groupStart, groupEnd do
            ...
            if groupSlot and groupSlot.class == slot.class and not occupied[j] then
                ...
                elseif groupSlot.class == "shaman" then
                    ...
                    if not groupSlot.extra or groupSlot.extra == "" then groupSlot.extra = SCB.DEFAULT_SHAMAN_TOTEMS end
                else
                    groupSlot.extra = roleInfo.extra
                end
            end
        ...
        if slot.class == "paladin" then
            if not slot.extra or slot.extra == "" then slot.extra = "BoK" end
        elseif slot.class == "shaman" then
            if not slot.extra or slot.extra == "" then slot.extra = SCB.DEFAULT_SHAMAN_TOTEMS end
        else
            slot.extra = roleInfo.extra
        end
    end
...
function SCB_PresetShamanTotemOnClick()
    ...
    list = SCB.SHAMAN_TOTEMS[groupKey]
...
function SCB_StartRefillAssignments(assignments)
    ...
    if table.getn(SCB.presetSpawnQueue) > 0
        ...
        return false, SCB_L("REPLACE_DEAD_BUSY")
    end
...
function SCB_RefillPresetOnClick()
    ...
    ok, errorText = SCB_StartRefillAssignments(missing)
...
function SCB_RefillOnUpdate(elapsed)
    ...
    if state.phase == "waitgroup" then
        ...
        if state.fixedAssignments then
            ...
            for i = 1, table.getn(state.fixedAssignments) do
                candidate = state.fixedAssignments[i]
                matched = false
                for j = 1, table.getn(state.assignments) do
                    if candidate == state.assignments[j] then matched = true break end
                end
                if not matched then table.insert(remainingAssignments, candidate) end
            end
            state.fixedAssignments = remainingAssignments
        end
